    ...
    <div class="grow w-full">
      <div class="text-center py-5 bg-white rounded-t-xl drop-shadow-2xl bg-opacity-80">
        <div class="mt-10 font-semibold text-lg  md:text-2xl text-[#8B2FC9]">
           ระบบรายงานสินค้า / ร้านค้า สินค้าตีกลับ 
        </div>
      </div>
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl" style="min-height: 82vh">
        ...
        >
          ...
          <Column header="สถานะ" >
            <template #body="item">
              <Chip :class="{
                'bg-[#f97316]': item.data.status === 'กำลังเตรียมจัดส่ง',
                'bg-[#6366f1]': item.data.status === 'จัดส่งสินค้า',
                'bg-[#119c8d]': item.data.status === 'ได้รับสินค้าแล้ว',
		        'bg-[#22c55e]': item.data.status === 'ส่งสินค้าแล้ว',
                'bg-red-500': item.data.status === 'ตีกลับสินค้า'
    ...
    <div class="w-full  md:mx-auto mt-5 pt-3 bg-[#fff] border border-[#8B2FC9] " >
        <div class=" text-xs md:text-lg  flex  items-center px-2 md:px-5 py-2 border-b border-gray-200">
            <div class="text-[#8B2FC9] flex-grow ">{{from.partner_name}} </div>    
              <div :class="{
                'text-[#f97316]': from.status === 'กำลังเตรียมจัดส่ง',
                'text-[#6366f1]': from.status === 'จัดส่งสินค้า',
                'text-[#119c8d]': from.status === 'ได้รับสินค้าแล้ว',
		        'text-[#22c55e]': from.status === 'ส่งสินค้าแล้ว',
                'text-red-500': from.status === 'ตีกลับสินค้า'
...
const getData = async () => {
    const config = {
      ...
      url: `${process.env.VUE_APP_MARUEY}reportreturnproduct/bypartner/${store.getters._id}`,
    };
