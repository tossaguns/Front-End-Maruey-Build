      ...
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl" style="min-height: 82vh">
        ...
        >
          ...
          <Column header="วันที่" >
            <template #body="item">
                <div v-if="status == 1" >{{ dayjs(item.data._id).format("DD/MM/YYYY") }}</div>
                <div v-else-if="status==2"> {{ dayjs(item.data._id).format("MM/YYYY") }}</div>
                <div v-else-if="status==3"> {{ dayjs(item.data._id).format("YYYY") }}</div>
            </template>
          </Column>
...
const getData = async () => {
    const config = {
      ...
      data: { id:"day",partner_id:store.getters._id},
      url: `${process.env.VUE_APP_MARUEY}report/reportpartnerbyid`,
    };
...
const getMonth = async () => {
    const config = {
      ...
      data: { id:"month",partner_id:store.getters._id},
      url: `${process.env.VUE_APP_MARUEY}report/reportpartnerbyid`,
    };
    await axios(config).then((res) => {
            data.value = res.data?.data;
...
const getYear  = async () => {
    const config = {
      ...
      data: { id:"year",partner_id:store.getters._id},
      url: `${process.env.VUE_APP_MARUEY}report/reportpartnerbyid`,
    };
    await axios(config).then((res) => {
            data.value = res.data?.data;
