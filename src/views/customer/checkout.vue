    ...
    <div class="h-full py-3 mx-2 lg:mx-0">
      ...
      <div class="lg:w-4/6 mx-2 md:mx-auto mt-5 pt-5 bg-[#fff] shadow mb-14">
        ...
        >
          ...
          <div class="text-right ml-3">{{ (shippingCost+tracking_price)?.toLocaleString("en", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              }) }}
...
const discount = computed(() => {
    ...
    let bonus = 0
    if (pv_upgrade >= 1200) {
        dis = Math.floor(pv_total* 50/100)
        bonus = 50
    } else {
...
const getstore = async () => {
  partner_id.value = [];
  const product = store.getters.cart?.filter(p => p.product_store !== 'partner');
  ...
  const Maruay = product.find((item) => item.product_store !== "partner");
  // เช็ค product_partner_id ว่ามีกี่ เจ้า

  if (Maruay) {
    partner_id.value.push({
      partner_id: null,
      partner_name: "บริษัท มารวยด้วยกัน จำกัด",
    });
  }
...
const addorder = async () => {
  if (product_id_fk.value?.length === 0) {
    return;
  }
  const result = await Swal.fire({
  ...
  if (result.isConfirmed) {
    ...
    if (addressType.value === "system") {
      data = {
        delivery_province_id: addressDelivery.value.province,
        house_no: addressDelivery.value.address,
        moo: addressDelivery.value.moo,
        soi: addressDelivery.value.soi,
        road: addressDelivery.value.road,
        tambon_id: addressDelivery.value.tambon,
        district_id: addressDelivery.value.district,
        province_id: addressDelivery.value.province,
        zipcode: addressDelivery.value.zipcode,
        phone: myFormatAddress.value.phone,
        name: myFormatAddress.value.name,

        address_sent: "system",
        house_name: "system",

        customer_id_fk: userdata.value.id,
        customer_user_name: userdata.value.user_name,
        tracking_type: tracking_type.value,

        product_id_fk: product_id_fk.value,
        amt: product_amt.value,
        quantity: quantity.value,
      };
    } else {
      data = {
        delivery_province_id: newAddress.value.province?.code,
        house_no: newAddress.value.address,
        moo: newAddress.value.moo,
        soi: newAddress.value.soi,
        road: newAddress.value.road,
        tambon_id: newAddress.value.tambon?.id,
        district_id: newAddress.value.amphure?.code,
        province_id: newAddress.value.province?.code,
        zipcode: newAddress.value.zipcode,
        phone: newAddress.value.phone,
        name: newAddress.value.receiver,
    ...
    const config = {
      method: "post",
      data: data,
      url: `${process.env.VUE_APP_MLM}/payment_submit`,
    };
    await axios(config)
      .then(async (res) => {
...
const product_id_fk = computed(() => {
  let result = [];
  const products = store.getters.cart?.filter(p => p.product_store !== 'partner')?.map((x) => x.product_id + "");
  result = products;
  return result;
});

const product_partners = computed(() => {
  let result = [];
  const products = store.getters.cart?.filter(p => p.product_store === 'partner')?.map((x) => {
    return {
      _id: x.product_id,
      shop_id: x.product_partner_id,
      qty: x.product_qty,
      unit_price: x.product_price,
      total_price: x.product_totalprice,
      unit_pv: x.product_pv,
      total_pv: x.product_pv*x.product_qty,
      name: x.product_name,
      image: 'file/'+x.product_image?.split('/file/')[1],
      store: x.product_store,
    }
  });
  result = products;
  return result;
