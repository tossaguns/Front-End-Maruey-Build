    ...
    <div class="h-full  min-h-screen  flex flex-col bg-[#f7f7f7] mb-20">
        <div class="flex">
            <div class="w-auto pl-4 py-3 ">
                <button class="bg-red-500 text-sm  text-[#fff] font-bold py-2 px-2 rounded flex" @click="router.push('/customer/customerview')">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
                     กลับหน้าข้อมูลลูกค้า
                </button>
            </div>
        </div>
        <div class="grid  grid-cols-2  lg:grid-cols-3  xl:grid-cols-6 gap-4 mt-2  mx-2 md:mx-0 ">
                    <div v-for="(product, _id) in data" :key="_id"  class=" bg-[#fff]" >
                        ...
                        <div class="border px-2" >
                            ...
                            <div class="flex  md:flex-row items-center mt-2.5 mb-4" @click="linkproduct(product._id)">
                                <div class="flex ">
                            ...
                            <div class="mx-1 my-2 text-base flex justify-center items-center" >
                                 <div class="w-1/2 text-xs md:text-base">
                                    ฿ {{product.product_price}}
                                </div>
                                <div class="w-1/2 text-right text-sm">
                                    <button @click="addproduct(product,1,'')" class="bg-[#8B2FC9] hover:bg-[#6818A5] rounded-md p-1.5 sm:p-2 text-[#fff] " v-if="product?.product_stock >0">
                                        <div class="flex items-center justify-center">
                                            <div class="text-xs"> เพิ่มลงตะกร้า</div>
                                        </div>
                                    </button>
                                    <div v-else>
                                        <div class="text-red-500 p-2 text-xs">สินค้าหมด !!</div>
                                    </div>
                                </div>
...
onMounted(async () => {
    ...
    document.title = "Maruey || เซตอัพเกรดสินค้า";
