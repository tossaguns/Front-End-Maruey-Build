<template>
      ...
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl" style="min-height: 82vh">
        ...
        >
          ...
          <Column header="รายได้" >
            <template #body="item">
              <div v-if="item.data.shareincome_id != null"> ฿ {{item.data?.shareincome_id?.maruey?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}}</div>
            </template>
          </Column>
  ...
  <Dialog v-model:visible="dialog" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"  >
    ...
    <div class="flex flex-col md:flex-row p-2  md:py-5 md:pl-5 text-base  md:text-lg  text-[#8B2FC9] border border-[#8B2FC9] ">
            ...
            <div class="w-full flex pb-2 md:pb-0 ">รายได้ : ฿{{from?.shareincome_id?.maruey?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}}</div>
    </div>   
...
const getData = async () => {
    const config = {
      ...
      url: `${process.env.VUE_APP_MARUEY}delivery/bypartnernamemaruey/`,
    };
