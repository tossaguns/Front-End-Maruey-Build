<template>
    ...
    <div class="grow w-full">
      ...
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl" style="min-height: 82vh">
        ...
        >
          ...
          <Column header="หมายเลขสั่งซื้อ" field="orderref" />
          <Column header="วันที่สั่งซื้อ" >
            <template #body="item">
              <div>{{dayjs(item.data?.createdAt).format("DD/MM/YYYY HH:mm")}}</div>
            </template>
          </Column>
          ...
          <Column header="ตัวเลือก">
            <template #body="item">
              <Button
                ...
                class="bg-[#8B2FC9]  hover:bg-[#5e2e7e] py-2.5 px-4 text-[#fff] w-full md:w-48"
  ...
  <Dialog v-model:visible="dialog" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"  >
    ...
    <div class="flex flex-col md:flex-row p-2  md:py-5 md:pl-5 text-base  md:text-lg  text-[#8B2FC9] border border-[#8B2FC9] ">
            ...
            <div class="w-full flex pb-2 md:pb-0 ">ชำระเงินทาง :{{from.payment}}</div>
    </div>   
    <div class="w-full  md:mx-auto mt-5 pt-3 bg-[#fff] border border-[#8B2FC9] " v-for="item  in from.suborder" :key="item._id">
        <div class=" text-xs md:text-lg  flex  items-center px-2 md:px-5 py-2 border-b border-gray-200">
            <div class="text-[#8B2FC9] flex-grow ">{{item.partner_name}} </div>    
              <div :class="{
                'text-[#f97316]': item.delivery_id.status === 'กำลังเตรียมจัดส่ง',
                'text-[#6366f1]': item.delivery_id.status === 'จัดส่งสินค้า',
                'text-[#119c8d]': item.delivery_id.status === 'ได้รับสินค้าแล้ว',
                'text-red-500': item.delivery_id.status === 'ตีกลับสินค้า'
              }">
                {{item.delivery_id.status}}
              </div>
            </div>
            
            <div class="mt-6 mb-5 pl-1 flex items-center ml-2 mr-5 md:mx-5"  v-for="product in item.product" :key="product.product_id">
            ...
            <div class="w-full flex items-center bg-[#fafdff] py-3 px-5  border-t border-gray-200  text-sm md:text-base">
                ...
                <div class="text-right md:w-2/12 w-4/12">฿ {{item?.totalproduct?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
            </div>
            <div class="w-full flex items-center bg-[#fafdff] py-3 px-5  text-sm md:text-base ">
                ...
                <div class="text-right md:w-2/12 w-4/12">฿ {{item?.delivery?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
            </div>
            <div class="w-full flex items-center bg-[#fafdff] p-5  border-t border-gray-200  text-base md:text-lg">
                ...
                <div class="text-right md:w-2/12 w-6/12">฿ {{item?.alltotal?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
            </div>

                    <div class="w-full flex  items-center bg-[#fafdff] pt-5 px-5   border-t-2 border-[#8B2FC9]  text-sm md:text-lg">
                       <div class="flex flex-col md:flex-row  w-full ">
                            <div class="md:flex-grow  text-center pb-2 md:pb-0">ที่อยู่ในการจัดส่ง</div>
                            <div class="text-sm  md:text-base"> เลขทริคกิ้ง : {{item.delivery_id.tracking}} </div>
                       </div>
                    </div>
                    <div class="flex flex-col md:flex-row w-full py-5">
                        <div class="w-full  md:w-3/12 pl-5 pr-3 text-xs ">
                           <div>{{item.delivery_id.address?.namedelivery}} </div>
                           <div>{{item.delivery_id.address?.telephone}}</div>
                           <div>{{item.delivery_id.address?.address +" "+item.delivery_id.address?.tambon + " "+item.delivery_id.address?.amphure+ " "+item.delivery_id.address?.province+ " "+item.delivery_id.address?.zipcode}}</div>
                        </div>
    ...
    <div class="w-full md:mx-auto mt-5 pt-5 bg-[#fff] border border-[#8B2FC9]  mb-14 text-sm  md:text-base">
                   <div class="flex w-full  px-5 pt-5 pb-3  " >
                        <div class="md:text-right md:flex-grow  w-8/12">รวมการสั่งซื้อ: </div>  
                        <div class="text-right md:w-2/12 w-4/12">฿{{ from.total?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                   </div>
                   <div class="flex w-full px-5 pb-3" >
                        <div class="md:text-right md:flex-grow  w-8/12 ">รวมการจัดส่ง: </div>  
                        <div class="text-right md:w-2/12 w-4/12">฿{{ from.totaldelivery?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                   </div>
                   <div class="flex w-full px-5 pb-3" >
                        <div class="md:text-right md:flex-grow  w-8/12 ">ส่วนลด: </div>  
                        <div class="text-right md:w-2/12 w-4/12">฿{{ from.totaldiscount?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                   </div>
                   <div class="flex w-full px-5 pb-3" >
                        <div class="md:text-right md:flex-grow  w-8/12">ใช้ Maruey Coins: </div>  
                        <div class="text-right md:w-2/12 w-4/12">฿{{ from.totalmarueycoin?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                   </div>
                   <div class="flex w-full px-5 pb-8 " >
                        <div class="md:text-right md:flex-grow  w-8/12 ">ยอดชำระเงินทั้งหมด </div>  
                        <div class="text-right md:w-2/12 w-4/12  text-base md:text-lg text-[#8B2FC9]">฿{{ from.alltotal?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                   </div>
...
const getData = async () => {
    const config = {
      ...
      url: `${process.env.VUE_APP_MARUEY}order/`,
    };
...
const opendialog = (item) => {
    ...
    headform.value = "หมายเลขคำสั่งซื้อ #"+from.value.orderref;
    dialog.value = true;
};
