  ...
  <div class="flex w-full pt-5 justify-center px-2 sm:px-10">
    <div class="grow w-full">
      ...
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl" style="min-height: 82vh">
        ...
        >
          <template #empty>
            <p class="font-italic text-center text-2xl">ไม่มีข้อมูลหมวดหมู่สินค้า</p>
          </template>
          <template #header>
          ...
          <Column header="หมวดหมู่สินค้า" field="category_name" />
  ...
  <Dialog v-model:visible="dialog" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"  >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
      <div class="col-span-full">
        <label class="block text-sm font-medium text-gray-700">ชื่อหมวดหมู่สินค้า :</label>
        <input
          v-model="from.category_name"
          type="text"
    ...
    <div class="flex items-center justify-center w-full">
      <button @click="headform == 'เพิ่มข้อมูลหมวดหมู่สินค้า' ? adddata() : editdata()"  :disabled="isLoading"   class="mt-9 font-semibold leading-none text-white py-4 px-10 bg-[#8B2FC9] rounded-lg hover:bg-[#E178C5] focus:ring-2 focus:ring-offset-2 focus:ring-[#8B2FC9] focus:outline-none mx-5">
        บันทึก
      </button>
...
const opendialog = (type, item) => {
    if (type === "edit") {
        ...
        headform.value = "แก้ไขข้อมูลหมวดหมู่สินค้า";
    } else {
        from.value = {
            category_name: "",
        };
        headform.value = "เพิ่มข้อมูลหมวดหมู่สินค้า";
    }
    dialog.value = true;
};
