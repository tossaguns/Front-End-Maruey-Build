<template>
  ...
  <div class="flex w-full pt-5 justify-center px-2 sm:px-10  "  >
    <div class="overflow-x-auto w-full">
      ...
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl overflow-x-auto" style="min-height: 82vh">
        ...
        >
          <template #empty>
            <p class="font-italic text-center text-2xl">ไม่มีข้อมูลสินค้า</p>
          </template>
          <template #header>
            <div class="flex flex-col md:flex-row  gap-2 justify-end">
              ...
              <span class="relative">
                ...
                :options="optionscarchpartner"
                optionLabel="company_name"
                optionValue="_id"
                placeholder="ค้นหาสินค้าด้วยชื่อ บริษัท"
              ...
              <span class="relative">
                ...
                :options="optionscarchcategory"
                optionLabel="category_name"
                optionValue="_id"
                placeholder="ค้นหาสินค้าด้วยหมวดหมู่"
                class="w-full border focus:outline-none"
              ...
              <span class="relative">
                ...
                <input
                  v-model="searchall"
                  placeholder="ค้นหา"
                  class=" pl-10 border rounded p-1 py-2  w-full" 
                />
              </span>
            </div>
          </template>
          <Column header="สถานะสินค้า" >
            <template #body="item">
                <div v-if="item.data.product_status !=null ">
                    <InputSwitch v-model="item.data.product_status"  @click="changestatus(item.data.product_status,item.data._id)" />
                </div>
         ...
         <Column header="จำนวนสต็อก" >
            <template #body="item">
              <div v-if="item.data.product_stock !=null "> {{ item.data.product_stock }} ชิ้น </div>
              <div v-else> ไม่มีสต็อก </div>
            </template>
          </Column>
  ...
  <Dialog v-model:visible="dialog" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"  >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
        ...
        <div class="col-span-full">
            <label class="block text-sm font-medium text-gray-700 mb-2">รูปภาพสินค้า</label>
             <span v-if="headform != 'เพิ่มข้อมูลสินค้า'">
    ...
    <div class="flex items-center justify-center w-full">
      <button @click="headform == 'เพิ่มข้อมูลสินค้า' ? adddata() : editdata()"  :disabled="isLoading"   class="mt-9 font-semibold leading-none text-white py-4 px-10 bg-[#8B2FC9] rounded-lg hover:bg-[#E178C5] focus:ring-2 focus:ring-offset-2 focus:ring-[#8B2FC9] focus:outline-none mx-5">
        บันทึก
      </button>
...
onMounted(async () => {
    document.title = "ข้อมูลสินค้า || Maruey";
    ...
    await axios(configpartner).then((res) => {
            ...
            if(res.data?.data.length > 0){
              optionscarchpartner.value  = res.data?.data;
              optionscarchpartner.value.push({_id:1,company_name:"บริษัท มารวยด้วยกัน จำกัด"});
              optionscarchpartner.value.push({_id:0,company_name:"ทั้งหมด"});            
              
              optionpartner.value.reverse();
            }
    ...
    await axios(configcategory).then((res) => {
            ...
            if(res.data?.data.length > 0)
            {
              optionscarchcategory.value = res.data?.data;
              optioncategory.value.push({_id:0,category_name:"ทั้งหมด"});
              optioncategory.value.reverse(); 
            }
...
const optionscarchpartner = ref([]);
const optionscarchcategory = ref([]);
const optionscarchtype = ref([]);
const searchall = ref(""); 
const searchpartner = ref("");
const searchcategory = ref("");
const Filter = computed(() => {
   if(searchall.value !='' || searchpartner.value !='' || searchcategory.value !='')
    {
        return data.value.filter((item) => {
            ...
            {
              if(searchpartner.value == 1)
              {
                return item.product_store == "Maruay";
              }
              if(searchpartner.value ==0)
              {
                return item
              }

              return item.product_partner_id?._id == searchpartner.value;
            }else{
              return item;
            }
        ...
        .filter((item) => {
          ...
          if(searchcategory.value !='')
          {
            ...
            return item.product_category?._id == searchcategory.value;
          }else{
            return item;
          }
...
const opendialog = (type, item) => {
    if (type === "edit") {
        ...
        headform.value = "แก้ไขข้อมูลสินค้า";
    } else {
        from.value = {
            _id:"",
            product_name:"",
            product_status_type: "สินค้าของตัวเอง",
            product_category: "",
            product_type: "",
            product_costprice: 0,
            product_price: 0,
            ...
            product_long: 0,
            product_height: 0,
            product_store: "Maruay",
            product_partner_id:null,
            product_detail: "",
            product_stock: 0,
            product_status: false,
            product_image: "",
        ...
        headform.value = "เพิ่มข้อมูลสินค้า";
    }
    dialog.value = true;
};
...
const adddata = async () => {
  ...
  if (from.value.product_name == "" || from.value.product_status_type == "" || from.value.product_category == "" || from.value.product_type == "" || from.value.product_price == 0 || from.value.product_store == "" || from.value.product_detail == "" || from.value.product_stock == null || from.value.product_costprice == 0 || from.value.product_weight == 0 || from.value.product_width ==0 || from.value.product_long == 0 || from.value.product_height == 0  || ( from.value.product_status_type =="ฝากขาย" && from.value.product_partner_id =='') ) {
        toast.add({
            severity: "error",
...
const editdata = async () => {
    ...
    if(result.isConfirmed){
        ...
        const config = {
            ...
            data: from.value,
            url: `${process.env.VUE_APP_MARUEY}product/${from.value._id}`,
        };
        await axios(config).then(async(res) => {
            if (res.data?.status === true) {
              if(from.value.product_image !='')
...
const uploadimgproduct = async (id) => {
    ...
    const config = {
        ...
        data: formData,
        url: `${process.env.VUE_APP_MARUEY}product/image/${id}`,
    };
    await axios(config).then((res) => {
        if (res.data?.status === true) {
...
const deletedata = async (id)=>{
    const result = await Swal.fire({
        text: "คุณต้องการลบข้อมูลสินค้าใช่หรือไม่?",
        icon: "question",
        iconColor: "#8B2FC9",
        showCancelButton: true,
        confirmButtonColor: "#8B2FC9",
    ...
    if(result.isConfirmed){
        ...
        const config = {
            ...
            url: `${process.env.VUE_APP_MARUEY}product/${id}`,
        };
        await axios(config).then(async(res) => {
            if (res.data?.status === true) {
                toast.add({
...
const changestatus = async(status,id)=>{
    ...
    const config = {
            ...
            url: `${process.env.VUE_APP_MARUEY}product/status/${id}`,
        };
        await axios(config).then(async(res) => {
            if (res.data?.status === true) {
                toast.add({
