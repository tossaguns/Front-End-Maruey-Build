    ...
    <div class="grow w-full">
      ...
      <div class="text-center py-5 bg-white drop-shadow-2xl bg-opacity-80 rounded-b-xl" style="min-height: 82vh">
        ...
        >
          ...
          <Column header="รายรับ">
            <template #body="item">
              <div> ฿ {{ item.data.income?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
            </template>
          </Column>
          <Column header="รายจ่าย">
            <template #body="item">
              <div> ฿ {{ item.data.expenses?.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
            </template>
          </Column>
...
const getData = async () => {
    const config = {
      ...
      data: { id:"day"},
      url: `${process.env.VUE_APP_MARUEY}report/reportprofitandloss`,
    };
