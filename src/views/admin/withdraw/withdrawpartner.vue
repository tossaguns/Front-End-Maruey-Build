...
const getData = async () => {
    const config = {
      ...
      url: `${process.env.VUE_APP_MARUEY}withdrawmoney/partnerbystatusfalse`,
    };
...
const getall = async () => {
    const config = {
      ...
      url: `${process.env.VUE_APP_MARUEY}withdrawmoney/partner`,
    };
    await axios(config).then((res) => {
            data.value = res.data?.data;
            if(data.value.length>0)
...
onMounted(async () => {
    document.title = "ข้อมูลการถอนเงิน partner || Maruey";
    isLoading.value = true;
    await getData();
    isLoading.value = false;
});
